...
def go(speed: float, turn: float):
    ...
    frontLeftMotor.setVelocity(10 * leftSpeed)
    backLeftMotor.setVelocity(10 * leftSpeed)

    frontRightMotor.setVelocity(10 * rightSpeed)
    backRightMotor.setVelocity(10 * rightSpeed)
...
def goTo(x: float, y: float, c: bool = True):
    ...
    if close and c:
        if abs(imu.angleToBall) < 0.1:
            ...
            go(0.2 * abs(imu.angleToBall / 22), -2 if imu.angleToBall < 0 else 2)
        return
...
class IMU:
    ...
    def step(self, timestep: int):
        ...
        self.canFeelBall = self.tickCount - self.wasBallClose < 1 and (
        ...
        if abs(getAngleDiff(newRotation, self.rotation)) > 95:
...
while robot.step(TIME_STEP) != -1:
    ...
    if not imu.canBeTrusted:
        frontLeftMotor.setVelocity(10)
        backLeftMotor.setVelocity(-10)
        frontRightMotor.setVelocity(10)
        backRightMotor.setVelocity(-10)
        continue
    ...
    match MODE:
        ...
        case "WAITING":
            ...
            elif (ball_dist < 0.7) or (abs(ball_angle) > 30):
                if error_count > 0:
                    error_count -= 1
            ...
            elif canon_time > 15 and not attacked and sqrt(imu.absoluteBallX ** 2 + imu.absoluteBallY ** 2) < 0.2:
                attacked = True
                MODE = "KICKOFF"
                if step < 0:
