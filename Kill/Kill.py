...
def main():
    ...
    def doTask(task: Literal["stabilize", "defend", "ddefend", "kill", "findBall", "pushBall", "pinBall"]):
    ...
    while robot.step(timestep) != -1:
        ...
        #print(f"Enemy   : {imu.enemyX}, {imu.enemyY}")
        #print(f"Rotation: {imu.getRotation()}")
        #print(f"Spins: {imu.spins}")
        #print(f"Ball PosR: {imu.relativeBallX}, {imu.relativeBallY}")
        #print(f"Ball PosA: {imu.absoluteBallX}, {imu.absoluteBallY}")
        #print(f"Ball Ang : {imu.angleToBall}")
        #print(f"Ball Dst : {imu.distanceToBall}")
        #print(f"Ball Vel : {imu.ballVelocityX}, {imu.ballVelocityY}")
        #print(f"Ball PosF: {imu.absoluteBallX + 0.5 * imu.ballVelocityX}, {imu.absoluteBallY + 0.5 * imu.ballVelocityY}")
        #print(f"Blind: {not imu.canSeeBall}")

        if not imu.canBeTrusted:
            doTask("stabilize")

        if imu.knowsWhereBallIs() and imu.absoluteBallX > 0.1 and task != "ddefend":
            task = "defend"

        if imu.knowsWhereBallIs() and imu.absoluteBallX < -0.1:
            task = "pushBall" if imu.x < imu.absoluteBallX else "ddefend"
        
        if not imu.knowsWhereBallIs() and task != "kill" and task != "ddefend":
            task = "findBall"
